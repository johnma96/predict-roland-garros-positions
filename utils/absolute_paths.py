...
class AbsPaths:
    ...
    def get_abs_path_file(self, file_name: str, deep: int = 5) -> Union[str, list]:
        ...
        if file_name in file_lev:
            possible_index = [
                index for (index, item) in enumerate(file_lev) if item == file_name
            ]
    ...
    def __build_paths(self) -> dict:
        ...
        def paths_at_level(level: int, type: str = "folders") -> list:
            ...
            if type == "folders":
                options = [
                    ...
                    if (
                        os.path.isdir(opt)
                        and opt.split(os.sep)[-1] not in self._excluded_folders
                    )
                ]
                    ...
                    if (
                        ...
                        and not (
                            any(
                                [
                                    opt.endswith(end)
                                    for end in self._file_endings_excluded
                                ]
                            )
                        )
                    )
        ...
        dict_paths_folders = {
            level: paths_at_level(level, type="folders")
            for level in range(0, self.max_level + 1)
        }
        dict_paths_files = {
            level: paths_at_level(level, type="files")
            for level in range(0, self.max_level + 1)
        }
    ...
    def excluded_folders(self, folder_list) -> None:
        ...
        if not isinstance(folder_list, list):
            raise ValueError(
                "You must pass a list of folders inside the package"
    ...
    def excluded_files(self, file_list) -> None:
        ...
        if not isinstance(file_list, list):
            raise ValueError(
                "You must pass a list of generic files inside the"
